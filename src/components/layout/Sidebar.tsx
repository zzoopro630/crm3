...
const navItems: NavItem[] = [
    ...
    { 
        ...
        isSubmenu: true,
        submenuItems: [
            { title: '조직 관리', href: '/settings/organizations', icon: Settings, allowedLevels: ['F1'] },
            { title: '라벨 관리', href: '/settings/labels', icon: Settings, allowedLevels: ['F1'] },
            { title: '사원 관리', href: '/settings/employees', icon: Settings, allowedLevels: ['F1'] },
            { title: '승인 대기', href: '/settings/approvals', icon: Settings, allowedLevels: ['F1'] },
...
export function Sidebar({ isOpen, onToggle, isCollapsed = false, onCollapseToggle }: SidebarProps) {
    ...
    const getActiveItem = (): NavItem | null => {
        for (const item of navItems) {
            if (item.submenuItems) {
                const activeSubItem = item.submenuItems.find(subItem => 
                    location.pathname === subItem.href || location.pathname.startsWith(subItem.href + '/')
                )
    ...
    const visibleNavItems = navItems.filter(item => {
        ...
        if (item.submenuItems) {
            item.submenuItems = item.submenuItems.filter(subItem => {
                if (!employee) return false
                if (subItem.allowedLevels && !subItem.allowedLevels.includes(employee.securityLevel)) return false
                return true
            })
        }
                ...
                <div className="flex h-full flex-col">
                    ...
                    <div className="flex h-16 items-center justify-between px-4 border-b border-border">
                        <div className="flex items-center gap-3">
                            ...
                            {!isCollapsed && <span className="text-lg font-semibold text-foreground">CRM</span>}
                        </div>
                    ...
                    <nav className="flex-1 p-4 space-y-1">
                        {visibleNavItems.map((item) => {
                            const isActive = location.pathname === item.href || (item.submenuItems && isInSubmenu(item))
                            const hasSubmenu = item.submenuItems && item.submenuItems.length > 0
                            const isSubmenuActive = item.submenuItems?.some(subItem => location.pathname === subItem.href)
                                ...
                                <div key={item.href}>
                                    ...
                                    <Link
                                        ...
                                        className={cn(
                                            ...
                                            activeItem && !isActive && activeItem.href !== item.href && !item.submenuItems?.some(sub => sub.href === activeItem.href)
                                                ? 'opacity-50'
                                                : ''
                                        )}
                                    >
...
export function SidebarCollapseTrigger({ isCollapsed, onToggle }: { isCollapsed: boolean; onToggle: () => void }) {
    return (
        <Button
            variant="ghost"
            size="icon"
